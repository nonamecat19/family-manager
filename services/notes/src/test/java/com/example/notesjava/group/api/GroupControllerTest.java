...
class GroupControllerTest {
    ...
    void createReturns201WithALocationHeader() throws Exception {
        ...
        mockMvc.perform(post("/api/groups").with(CALLER).contentType(MediaType.APPLICATION_JSON)
    ...
    void rejectsAnUpdateWithoutAColour() throws Exception {
        mockMvc.perform(put("/api/groups/1").with(CALLER).contentType(MediaType.APPLICATION_JSON)
                        .content("{\"title\":\"work\"}"))
                .andExpect(status().isBadRequest())
    ...
    void missingGroupBecomesA404ProblemDetail() throws Exception {
        ...
        mockMvc.perform(get("/api/groups/404").with(CALLER))
