// Command server runs the notes service: Connect/JSON on :8085 for apps, gRPC on :9095 for
...
import (
	...
	"github.com/nnc/family-manager/services/notes/internal/config"
	dbfs "github.com/nnc/family-manager/services/notes/internal/db"
	notesevents "github.com/nnc/family-manager/services/notes/internal/events"
	"github.com/nnc/family-manager/services/notes/internal/handler"
	"github.com/nnc/family-manager/services/notes/internal/store"
)
...
func run() error {
	...
	log := logger.New(logger.Options{Service: "notes", Level: cfg.LogLevel, JSON: cfg.LogJSON})
	...
	bus, err := events.Connect(events.Config{URL: cfg.NATSURL, Name: "notes"})
	if err != nil {
		...
		defer bus.Close()
		if err := bus.EnsureStream(ctx, notesevents.Domain); err != nil {
	...
	// Notes has no internal-only procedures: every procedure takes family_id and user_id from
	// the token claim, none from the request body. The gRPC listener is the same handler
	// without the auth interceptor, for sibling services on the private network.
...
func publicMux(h *handler.Handler, verifier *fmauth.Verifier, pool database.Pinger, log *slog.Logger) *http.ServeMux {
	...
	path, svc := notesv1connect.NewNotesServiceHandler(
...
func internalMux(h *handler.Handler, pool database.Pinger, log *slog.Logger) *http.ServeMux {
	...
	path, svc := notesv1connect.NewNotesServiceHandler(h,
		connect.WithReadMaxBytes(maxRequestBytes),
...
// maxRequestBytes bounds a decoded request body. UploadNoteImage caps the image itself at
...
// and an UpdateNote carrying a long note's whole block array.
...
func newServer(port string, mux *http.ServeMux) *http.Server {
	return &http.Server{
		...
		// slowest legitimate request — an 8 MiB image from a phone on a bad connection —
		// which makes them useless as a defence. ReadHeaderTimeout stops the slowloris that
		// matters, connect.WithReadMaxBytes bounds the body, and IdleTimeout reclaims
...
// means image upload runs in degraded mode (UploadNoteImage errors, everything else works)
...
func imagesOrNil(ctx context.Context, cfg *config.Config, log *slog.Logger) (handler.ImageStore, error) {
	if cfg.StorageEndpoint == "" {
		log.Warn("image storage disabled: NOTES_STORAGE_ENDPOINT not set")
		return nil, nil
	}
