...
export function FamilyProvider({ children }: { children: ReactNode }) {
  ...
  async function loadFamilies() {
    try {
      ...
      const storedFamilyId = localStorage.getItem('active_family_id');
      if (storedFamilyId) {
  ...
  async function setActiveFamily(family: Family) {
    ...
    setActiveFamilyState(family);
    localStorage.setItem('active_family_id', family.id);
    ...
    try {
      await apiClient.post('/families/switch', { familyId: family.id });
    } catch (error) {
